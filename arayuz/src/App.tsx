...
import { Activity, ArrowLeft, BarChart3, ChevronLeft, ChevronRight, ClipboardList, Clock, CreditCard, Eye, EyeOff, FileSpreadsheet, FileText, HardDrive, Landmark, Mail, MapPin, Pencil, Phone, RotateCcw, Search, Settings, TrendingUp, Trash2, Users, Wallet, X } from "lucide-react";
...
import { AramaKutusu, BirincilButon, GizliTutar, IkonButon, Panel, Sayfalama, SiralamaBaslik, bugun, para } from "./bilesenler/Yardimcilar";
...
function DashboardSayfasi({ onCariAc }: { onCariAc: (id: number) => void }) {
  const [dBlurCari, setDBlurCari] = useState(() => { try { return localStorage.getItem("db_blur_cari") === "true"; } catch { return false; } });
  const [dBlurAlacak, setDBlurAlacak] = useState(() => { try { return localStorage.getItem("db_blur_alacak") === "true"; } catch { return false; } });
  const [dBlurGrafikSatis, setDBlurGrafikSatis] = useState(() => { try { return localStorage.getItem("db_blur_grafik_satis") === "true"; } catch { return false; } });
  const [dBlurGrafikTahsilat, setDBlurGrafikTahsilat] = useState(() => { try { return localStorage.getItem("db_blur_grafik_tahsilat") === "true"; } catch { return false; } });
  const [dBlurTablo, setDBlurTablo] = useState(() => { try { return localStorage.getItem("db_blur_tablo") === "true"; } catch { return false; } });
  useEffect(() => { try { localStorage.setItem("db_blur_cari", dBlurCari ? "true" : "false"); } catch {} }, [dBlurCari]);
  useEffect(() => { try { localStorage.setItem("db_blur_alacak", dBlurAlacak ? "true" : "false"); } catch {} }, [dBlurAlacak]);
  useEffect(() => { try { localStorage.setItem("db_blur_grafik_satis", dBlurGrafikSatis ? "true" : "false"); } catch {} }, [dBlurGrafikSatis]);
  useEffect(() => { try { localStorage.setItem("db_blur_grafik_tahsilat", dBlurGrafikTahsilat ? "true" : "false"); } catch {} }, [dBlurGrafikTahsilat]);
  useEffect(() => { try { localStorage.setItem("db_blur_tablo", dBlurTablo ? "true" : "false"); } catch {} }, [dBlurTablo]);
  ...
  useEffect(() => {
    ...
    if (s) try {
      ...
      if (o.dPreset !== undefined) setDPreset(o.dPreset);
  ...
  const dashboardVeriGetir = () => api.dashboardGetir(baslangic, bitis).then(setVeri);
  useEffect(() => { dashboardVeriGetir(); }, [baslangic, bitis]);
  veriGuncellendiRef.current = () => { dashboardVeriGetir(); api.dashboardGetir("", "").then(setChartVeri); };
  const [chartVeri, setChartVeri] = useState<Dashboard | null>(null);
  useEffect(() => { dashboardVeriGetir(); api.dashboardGetir("", "").then(setChartVeri); const h = () => veriGuncellendiRef.current(); window.addEventListener("veri-guncellendi", h); return () => window.removeEventListener("veri-guncellendi", h); }, []);
      ...
      <div className="grid gap-4 xl:grid-cols-[320px_1fr]">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
          <div className="relative overflow-hidden rounded-[28px] border border-sky-200/60 bg-[linear-gradient(135deg,#0f172a,#2563eb)] p-6 text-white shadow-[0_24px_60px_rgba(37,99,235,0.25)]">
            <button type="button" onClick={() => setDBlurCari(b => !b)} className="absolute right-3 top-3 z-10 rounded-md border border-white/20 bg-white/10 p-1 text-white/70 transition hover:bg-white/20">{dBlurCari ? <EyeOff size={14} /> : <Eye size={14} />}</button>
            <p className="text-sm font-medium text-white/70">Toplam Müşteri Sayısı</p>
            <p className="mt-2 text-2xl font-bold tracking-tight">{dBlurCari ? "*,**" : (veri?.toplamCari ?? 0)}</p>
          ...
          <div className="relative overflow-hidden rounded-[28px] border border-rose-200/60 bg-[linear-gradient(135deg,#991b1b,#fb7185)] p-6 text-white shadow-[0_24px_60px_rgba(244,63,94,0.22)]">
            <button type="button" onClick={() => setDBlurAlacak(b => !b)} className="absolute right-3 top-3 z-10 rounded-md border border-white/20 bg-white/10 p-1 text-white/70 transition hover:bg-white/20">{dBlurAlacak ? <EyeOff size={14} /> : <Eye size={14} />}</button>
            <p className="text-sm font-medium text-white/70">Toplam Alacak</p>
            <p className="mt-2 text-2xl font-bold tracking-tight">{dBlurAlacak ? "*,**" : para(veri?.toplamAlacak ?? 0)}</p>
            <div className="absolute -bottom-2 -right-2 h-20 w-20 rounded-full bg-white/5" />
          </div>
        </div>
        <div className="grid gap-4 xl:grid-cols-2">
          <div className="relative overflow-hidden rounded-[28px] border border-slate-200 bg-gradient-to-br from-blue-100/60 via-white to-slate-100 p-5 shadow-[0_24px_60px_rgba(15,23,42,0.08)] dark:border-slate-800 dark:bg-gradient-to-br dark:from-blue-950/40 dark:via-slate-800 dark:to-slate-900">
            <button type="button" onClick={() => setDBlurGrafikSatis(b => !b)} className="absolute right-4 top-4 z-10 rounded-md border border-slate-200 bg-white p-1.5 text-slate-400 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-500">{dBlurGrafikSatis ? <EyeOff size={14} /> : <Eye size={14} />}</button>
            ...
            {grafikVeri.keys.length === 0 ? <p className="py-6 text-center text-sm text-slate-400">Henüz satış verisi yok</p> : dBlurGrafikSatis ? <div className="flex items-center justify-center py-10 text-3xl font-bold tracking-widest text-slate-300 dark:text-slate-600">*,**</div> : <div className="overflow-x-auto overflow-y-hidden pb-2">
          ...
          <div className="relative overflow-hidden rounded-[28px] border border-slate-200 bg-gradient-to-br from-blue-100/60 via-white to-slate-100 p-5 shadow-[0_24px_60px_rgba(15,23,42,0.08)] dark:border-slate-800 dark:bg-gradient-to-br dark:from-blue-950/40 dark:via-slate-800 dark:to-slate-900">
            <button type="button" onClick={() => setDBlurGrafikTahsilat(b => !b)} className="absolute right-4 top-4 z-10 rounded-md border border-slate-200 bg-white p-1.5 text-slate-400 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-500">{dBlurGrafikTahsilat ? <EyeOff size={14} /> : <Eye size={14} />}</button>
            <div className="mb-4 flex items-center justify-between gap-3">
            ...
            {grafikVeriTahsilat.keys.length === 0 ? <p className="py-6 text-center text-sm text-slate-400">Henüz tahsilat verisi yok</p> : dBlurGrafikTahsilat ? <div className="flex items-center justify-center py-10 text-3xl font-bold tracking-widest text-slate-300 dark:text-slate-600">*,**</div> : <div className="overflow-x-auto overflow-y-hidden pb-2">
          ...
          <div className="flex items-center justify-between px-5 pb-3 pt-3">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setDBlurTablo(b => !b)} className={"rounded-md border border-slate-200 p-1.5 text-slate-400 transition hover:bg-slate-100 dark:border-slate-700 dark:text-slate-500 dark:hover:bg-slate-700"}>{dBlurTablo ? <EyeOff size={14} /> : <Eye size={14} />}</button>
              ...
              {sayfaFiltreli.map((h, idx) => (
                <tr key={h.id} onClick={() => onCariAc(h.cariId)} className={`${tabloStil.satir} cursor-pointer`}>
                  ...
                  <td className={`${tabloStil.hucre} font-medium text-slate-800 dark:text-slate-200`}>{h.cariUnvan}</td>
                    <td className={tabloStil.hucre}><span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${h.belgeTuru === "Tahsilat" ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-300" : "bg-rose-50 text-rose-600 dark:bg-rose-900/30 dark:text-rose-300"}`}>{h.belgeTuru}<span className="opacity-70">(<GizliTutar deger={h.tutar} gizle={dBlurTablo} />)</span></span></td>
                  ...
                  <td className={`${tabloStil.hucre} whitespace-nowrap ${tabloStil.metin}`}>{tarihFormatla(h.tarih)} {(() => { const s=new Date(h.tarih.slice(0,10)+"T00:00:00"), t=Date.now(); const gf=Math.floor((t-s.getTime())/86400000); const y=Math.floor(gf/365), k=gf%365, a=Math.floor(k/30), g=k%30; const p=[]; if(y>0)p.push(y+' yıl'); if(a>0)p.push(a+' ay'); p.push(g+' gün'); const renk=gf>180?'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300':gf>60?'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300':gf>30?'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300':'bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-400'; return <span className={'ml-1 inline-block rounded-md px-1.5 py-0.5 text-[10px] font-semibold '+renk}>(İşlemden bu yana {p.join(' ')} geçti)</span>; })()}</td>
                    <td className={`${tabloStil.hucreSag} whitespace-nowrap text-sm font-semibold tabular-nums tracking-tight text-red-600 dark:text-red-400`}>{h.cariBakiye != null ? <GizliTutar deger={h.cariBakiye} gizle={dBlurTablo} /> : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function CariSayfasi({ onCariAc }: { onCariAc: (id: number) => void }) {
  const [cBlur, setCBlur] = useState(() => { try { return localStorage.getItem("cl_blur") === "true"; } catch { return false; } });
  useEffect(() => { try { localStorage.setItem("cl_blur", cBlur ? "true" : "false"); } catch {} }, [cBlur]);
          ...
          <tbody className={tabloStil.govde}>
            {sonuc.veriler.map((kayit) => (
              <tr key={kayit.id} onClick={() => onCariAc(kayit.id)} className={`${tabloStil.satir} cursor-pointer`}>
                ...
                <td className={`${tabloStil.hucreSag} font-semibold text-red-600 dark:text-red-400`}><GizliTutar deger={kayit.bakiye} gizle={cBlur} /></td>
